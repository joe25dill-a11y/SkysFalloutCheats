...
// Common 1.4.0.525 world SceneGraph pointer (validated by FOV range before use).
...
constexpr float kDefaultFovY = 75.f * kPi / 180.f; // vanilla world FOV, not 82
...
bool TryFovFromInterfaceSceneGraphs(float* outFovRad)
{
	__try {
		...
		// Prefer the larger FOV (world-ish) over tiny UI FOVs when both exist.
		if (ok0 && ok1) {
...
bool TryFovFromD3D(IDirect3DDevice9* device, float* outFovRad)
{
	...
	// Reject near-identity / garbage projections.
	if (std::fabs(proj._33) < 1.0e-5f && std::fabs(proj._43) < 1.0e-5f) return false;
	*outFovRad = fov;
	return true;
}

// Player REFR rot: yaw=rotZ, pitch=rotX (path that currently draws boxes).
...
void CaptureCameraForFrame(IDirect3DDevice9* device)
{
	...
	// Live FOV first (ALT zoom / iron sights). Never invent screen offsets.
	...
	// Eye from camera node (position only). Look from player rot — cam-node
	// rotation previously produced garbage pitch and killed boxes.
	g_cam.eyeX = px;
	g_cam.eyeY = py;
	g_cam.eyeZ = pz + 120.f;
	...
	if ((cool++ % 180) == 0) {
		...
		SFC_LOG("W2S src=%d fovSrc=%d fov=%.1f ok=%d xy=%.0f,%.0f pitch=%.1f vp=%.0fx%.0f",
			g_cam.source, g_cam.fovSource, g_cam.fovY * kDeg, ok ? 1 : 0,
			tsx, tsy, g_cam.pitch * kDeg, g_cam.sw, g_cam.sh);
	}
}
...
ScreenPos WorldToScreen(float wx, float wy, float wz)
{
	...
	__try {
		// Single projection path only — angular fallback caused look-away drift.
	...
	float& outMinX, float& outMinY, float& outMaxX, float& outMaxY)
{
	...
	// Feet at (cx,cy,cz); box center at mid-height. Axes: X width, Y depth, Z height.
	const float midX = cx;
	const float midY = cy;
